        ...
        <div className="relative h-screen overflow-hidden bg-slate-950 text-white">
            ...
            <div className="pointer-events-none absolute inset-0 z-10 flex flex-col justify-between p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="pointer-events-auto max-w-md rounded-3xl border border-white/10 bg-black/45 p-5 backdrop-blur-xl">
                        {isPreview ? (
                            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-sky-400/30 bg-sky-400/15 px-3 py-2 text-xs font-bold uppercase tracking-[0.18em] text-sky-200">
                                <Eye className="h-4 w-4" />
                                Vista previa docente
                            </div>
                        ) : null}
                        <p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-300">Reto 3D</p>
                        <h1 className="mt-2 text-4xl font-black text-white">{isPreview ? previewTitle : 'Ahorcado'}</h1>
                        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                            <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-zinc-500">Pista</p>
                            <p className="mt-2 text-sm leading-6 text-zinc-200">{content.clue || 'Sin pista configurada todavía.'}</p>
                        </div>
                        <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm text-amber-100">
                            <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-amber-200/80">Palabra oculta</p>
                            <p className="mt-2 font-semibold">{hiddenWordLength} letra{hiddenWordLength === 1 ? '' : 's'} para descubrir.</p>
                        </div>
                        {error ? (
                    ...
                    <div className="pointer-events-auto rounded-3xl border border-white/10 bg-black/45 p-5 backdrop-blur-xl">
                        <p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-400">Estado</p>
                        <div className="mt-3 grid grid-cols-2 gap-3 text-center">
                            <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3">
                                <p className="text-xs uppercase tracking-[0.18em] text-emerald-300/80">Aciertos</p>
                                <p className="mt-1 text-3xl font-black text-emerald-200">{guessedLetters.length}</p>
                            </div>
                            <div className="rounded-2xl border border-red-400/20 bg-red-400/10 px-4 py-3">
                                <p className="text-xs uppercase tracking-[0.18em] text-red-300/80">Fallos</p>
                                <p className="mt-1 text-3xl font-black text-red-200">{wrongLetters.length}/{MAX_ERRORS}</p>
                            </div>
                        </div>
                    </div>
                </div>
